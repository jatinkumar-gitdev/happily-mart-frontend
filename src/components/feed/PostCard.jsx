  ...
  return (
    <>
      <motion.div
        ...
        animate={{ opacity: isExpired ? 0.5 : 1, y: 0 }}
        transition={{ duration: 0.4 }}
        className={`relative bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-6 transition-all duration-300 flex flex-col ${
          isBlurred && !isUnlocked ? "post-blurred" : ""
        } ${viewMode === "grid" ? "h-full" : ""} ${isExpired ? "opacity-50 grayscale" : ""}`}
      >
        {/* Animated Background */}
        ...
        {/* Header */}
        <motion.div
          className="relative p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.1 }}
        >
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            <div className="flex-1 w-full sm:w-auto">
              <div className="flex flex-col gap-2">
                <motion.h3
                  className="font-bold text-gray-800 dark:text-white text-base sm:text-lg md:text-xl line-clamp-2"
                  whileHover={{ scale: 1.02 }}
                  transition={{ type: "spring", stiffness: 400 }}
                >
                  {post.title}
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {post.dealResult && ['Won', 'Failed'].includes(post.dealResult) && (
                    <div className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold text-white shadow-md ${
                      post.dealResult === 'Won' 
                        ? 'bg-gradient-to-r from-green-500 to-green-600' 
                        : 'bg-gradient-to-r from-red-500 to-red-600'
                    }`}>
                      <span className="text-sm">{post.dealResult === 'Won' ? '✓' : '✕'}</span>
                      <span>Deal {post.dealResult}</span>
                    </div>
                  )}
                  
                  {isOwnPost && contactCount > 0 && (
                    <div className="inline-flex items-center gap-1 bg-green-500 text-white px-2.5 py-1 rounded-full text-xs font-bold shadow-md">
                      <FaEye className="text-xs" />
                      <span>{contactCount} contacted</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Author Section */}
            <div className="flex items-center gap-2 w-full sm:w-auto sm:min-w-[200px] md:min-w-[250px]">
              <div className="flex items-center gap-2 flex-1 bg-gradient-to-r from-sky-600 via-sky-700 to-[--button-bg] rounded-full px-3 py-2">
                <img
                  src={getAvatarUrl(post.author?.avatar) || "/default-avatar.png"}
                  alt={post.author?.name || "Author avatar"}
                  className="w-7 h-7 sm:w-8 sm:h-8 rounded-full object-cover flex-shrink-0"
                />
                <p className="text-sm sm:text-base text-white font-semibold tracking-wide truncate flex-1">
                  {authorUnavailable ? "Unavailable" : post.author?.name}
                </p>
        ...
        <div className="relative p-3 sm:p-4 flex-grow">
          ...
          <motion.div
            ...
            transition={{ delay: 0.2 }}
          >
            <h4 className="font-semibold text-gray-700 dark:text-gray-300 text-base sm:text-lg mb-1">
              Requirement:
            </h4>
          ...
          >
            ...
            <div className="text-sm sm:text-base text-gray-600 dark:text-gray-400">
              {isUnlocked ? (
                ...
                <p>
                  {post.description?.substring(0, viewMode === "grid" ? 80 : 150) + "..."}
                </p>
              )}
            </div>
          </motion.div>
          ...
          {!isOwnPost && !isUnlocked && !authorUnavailable && post.creditCost > 0 && 
           post.postStatus !== "Expired" && !post.isExpired && 
           post.dealResult !== "Won" && post.dealResult !== "Failed" && (
            <motion.div
              className="mb-3 sm:mb-4 p-3 sm:p-4 bg-gradient-to-br from-sky-50 to-indigo-50 dark:from-sky-900/20 dark:to-indigo-900/20 rounded-xl border-2 border-sky-200 dark:border-sky-800"
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              transition={{ delay: 0.4 }}
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  >
                    <FiLock className="text-sky-600 dark:text-sky-400 text-lg sm:text-xl flex-shrink-0" />
                  </motion.div>
                ...
                <motion.button
                  ...
                  onHoverStart={() => setIsHovering(true)}
                  onHoverEnd={() => setIsHovering(false)}
                  className="relative w-full sm:w-auto px-4 sm:px-5 py-2 sm:py-2.5 rounded-full font-semibold text-xs sm:text-sm transition-all overflow-hidden shadow-lg hover:shadow-xl flex items-center justify-center min-w-[160px] sm:min-w-[180px] bg-gradient-to-r from-sky-500 to-[--button-bg] hover:from-sky-600 hover:to-indigo-700 text-white"
                  whileTap={{ scale: 0.95 }}
                >
                  <motion.div
                    className="flex items-center gap-2"
                    animate={isHovering ? { opacity: 0, scale: 0.8 } : { opacity: 1, scale: 1 }}
                    transition={{ duration: 0.2 }}
                  >
                    <FiUnlock />
                    <span>
                      Unlock ({post.creditCost || 1} Credit{(post.creditCost || 1) > 1 ? "s" : ""})
                    </span>
                  </motion.div>
                  ...
                  >
                    {[0, 1, 2].map((index) => (
                      <motion.div
                        key={index}
                        initial={{ x: -20, opacity: 0 }}
                        animate={isHovering ? { x: [0, 30], opacity: [0, 1, 0] } : { x: -20, opacity: 0 }}
                        transition={{
                          duration: 1.5,
                          repeat: isHovering ? Infinity : 0,
                          delay: index * 0.2,
          ...
          {/* Status Messages */}
          {isUnlocked && !isOwnPost && (
            <div className="mb-3 sm:mb-4 p-3 sm:p-4 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-xl text-xs sm:text-sm text-emerald-700 dark:text-emerald-300">
              <div className="flex items-center gap-2">
                <FiUnlock className="text-emerald-600 dark:text-emerald-400 flex-shrink-0" />
                <span>You've unlocked this post. Contact details are now visible.</span>
              </div>
            </div>
          )}
          ...
          {authorUnavailable && (
            <div className="mb-3 sm:mb-4 p-3 sm:p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl text-xs sm:text-sm text-red-700 dark:text-red-300">
              This post's owner is no longer available. Unlocking has been disabled.
            </div>
          )}

          {(post.postStatus === "Expired" || post.isExpired) && !isOwnPost && (
            <div className="mb-3 sm:mb-4 p-3 sm:p-4 bg-gray-100 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-xl text-xs sm:text-sm text-gray-700 dark:text-gray-300">
              <div className="flex items-center gap-2">
                <TbRefreshAlert className="flex-shrink-0" />
                <span>This post has expired. The creator can revive it to enable unlocking again.</span>
              </div>
            </div>
          )}
          ...
          <AnimatePresence>
            {isUnlocked && !isOwnPost && !authorUnavailable && post.author && (
              <motion.div
                className="mb-3 sm:mb-4 p-3 sm:p-4 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-xl border-2 border-green-200 dark:border-green-800"
                initial={{ opacity: 0, height: 0 }}
                animate={{ opacity: 1, height: "auto" }}
                exit={{ opacity: 0, height: 0 }}
                transition={{ duration: 0.5 }}
              >
                <motion.h4
                  className="font-semibold text-green-800 dark:text-green-400 mb-2 sm:mb-3 flex items-center gap-2 text-sm sm:text-base"
                  initial={{ x: -20 }}
                  animate={{ x: 0 }}
                >
                  <FiUnlock className="flex-shrink-0" />
                  Contact Details:
                </motion.h4>
                <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                  ...
                  {post.author.phone && (
                    <motion.p
                      ...
                      transition={{ delay: 0.2 }}
                    >
                      <span className="font-medium">Phone:</span> {post.author.phone}
                    </motion.p>
                  )}
                  {post.author.companyName && (
                    <motion.p
                      ...
                      transition={{ delay: 0.3 }}
                    >
                      <span className="font-medium">Company:</span> {post.author.companyName}
                    </motion.p>
                  )}
                </div>
              </motion.div>
        ...
        {/* Engagement Footer */}
        <motion.div
          className="relative p-3 sm:p-4 border-t border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mt-auto"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.5 }}
        >
          <div className="flex items-center gap-3 sm:gap-4 flex-wrap">
            ...
            >
              <motion.div
                ...
                transition={{ duration: 0.5 }}
              >
                <FaOpencart className={`text-lg sm:text-xl ${isFavorited ? "fill-current" : ""}`} />
              </motion.div>
              <span className="text-xs sm:text-sm font-medium">
                {post.favorites?.length || 0}
              </span>
            ...
            <motion.button
              onClick={handleShare}
              className="flex items-center gap-1.5 sm:gap-2 text-gray-600 dark:text-gray-400 hover:text-sky-500 transition-colors"
              whileHover={{ scale: 1.1 }}
              whileTap={{ scale: 0.9, rotate: -15 }}
            >
